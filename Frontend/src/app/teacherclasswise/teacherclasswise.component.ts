...
@Component({
  selector: 'app-teacherclasswise',
  standalone: true,
  imports: [FormsModule, CommonModule, NavbarLoggedComponent],
  templateUrl: './teacherclasswise.component.html',
  styleUrls: ['./teacherclasswise.component.css']
})
export class TeacherclasswiseComponent implements OnInit {
  email: string = '';
  role: string = '';
  ...
  sections: any[] = [];               // ✅ Now expects array of { id, classId }
  selectedSubjectCode: string = '';
  selectedSection: any = null;       // ✅ Now stores full section object
  ...
  fetchUserDetails() {
    this.http.get<any>("http://localhost:8082/user/details", this.getHttpOptions()).subscribe(
      user => {
        ...
        this.loadSubjects(); // Load subjects after fetching email
      },
      error => {
  ...
  loadSections() {
    this.classService.getAllClasses().subscribe(
      (data: any[]) => {
        ...
        this.sections = data; // ✅ Assign directly if no filtering needed
      },
      error => {
